...
export const DestinationHeroCarouselClient: React.FC<DestinationHeroCarouselClientProps> = ({
  ...
  const displayTitle = useMemo(() => {
    ...
    const formattedSlug = rawSlug.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')
  ...
  const [transitionStyle, setTransitionStyle] = useState(`transform ${TRANSITION_DURATION_MS}ms ease-out`)
  const [planeTransitionStyle, setPlaneTransitionStyle] = useState(`offset-distance ${TRANSITION_DURATION_MS}ms ease-out, opacity ${TRANSITION_DURATION_MS}ms ease-out`)
    ...
    if (transitionStyle === 'none') {
      const timer = setTimeout(() => {
        requestAnimationFrame(() => {
          ...
          setPlaneTransitionStyle(`offset-distance ${TRANSITION_DURATION_MS}ms ease-out, opacity ${TRANSITION_DURATION_MS}ms ease-out`)
        })
      }, 50)
  ...
  return (
    <section className="relative w-full bg-white font-sans overflow-hidden" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
      ...
      <h1 className="text-3xl md:text-4xl font-bold text-center mt-8 mb-6 text-gray-800">{displayTitle}</h1>

      <div className="relative w-full" style={{ height: '100px' }}>
        <svg className="absolute inset-x-0 -top-4 h-[340px] w-[2000px] max-w-none left-1/2 -translate-x-1/2 pointer-events-none" viewBox="0 0 2000 340" fill="none">
          <path id="flight-arc" d={arcPath} stroke="#9CA3AF" strokeWidth="3" strokeDasharray="6 8" fill="none" />
        </svg>

        <svg className="absolute inset-x-0 -top-4 h-[340px] w-[2000px] max-w-none left-1/2 -translate-x-1/2" viewBox="0 0 2000 340">
            ...
            return (
              <g key={`${stop.id}-${idx}`}>
                <foreignObject x="0" y="15" width="100%" height="100%" style={{ overflow: 'visible' }}>
                  <div style={{ offsetPath: `path('${arcPath}')`, offsetRotate: 'auto 90deg', offsetDistance: `${targetPosition * 100}%`, transition: planeTransitionStyle, position: 'absolute', opacity: opacity }}>
                    <PlaneIcon className={`w-6 h-6 md:w-7 md:h-7 transition-all duration-500 ${isCenter ? 'text-black scale-110' : 'text-gray-800 scale-100'} ${isOuter ? 'scale-90' : ''}`} style={{ transform: 'translateX(-50%) translateY(-80%)' }} />
                  </div>
                </foreignObject>
      ...
      <div className="relative pb-12 w-full">
        <Button onClick={scrollLeft} className="absolute left-4 top-1/2 -translate-y-1/2 w-[50px] h-[50px] bg-gray-200/75 rounded-full shadow-lg opacity-70 hover:opacity-100 transition-all z-50 flex items-center justify-center" aria-label="Previous destinations">
          <ChevronLeftIcon className="w-6 h-6" />
        </Button>
        <Button onClick={scrollRight} className="absolute right-4 top-1/2 -translate-y-1/2 w-[50px] h-[50px] bg-gray-200/75 rounded-full shadow-lg opacity-70 hover:opacity-100 transition-all z-50 flex items-center justify-center" aria-label="Next destinations">
          <ChevronRightIcon className="w-6 h-6" />
        </Button>

        <div className="overflow-hidden w-full">
          <div onTransitionEnd={handleTransitionEnd} style={{ width: 'max-content', transform: `translateX(-${transformValue}px)`, transition: transitionStyle }} className="flex items-end gap-0 pt-16 pb-12">
            {infiniteStops.map((stop, idx) => {
              const isCenter = idx === currentIndex

              return (
                <div key={`${stop.id}-${idx}`} className="flex justify-center items-end" style={{ width: `${centerCardWidth}px` }}>
                  <div className={`relative shrink-0 transition-all ease-out ${isCenter ? `w-[450px] h-[480px] translate-y-8` : `w-[280px] h-[340px]`}`} style={{ transitionDuration: `${TRANSITION_DURATION_MS}ms` }}>
                    <article className={`group rounded-lg overflow-hidden h-full flex flex-col shadow-lg ${isCenter ? 'bg-transparent' : 'bg-white'}`}>
                      {isCenter ? (
                        <>
                          <div className="w-full overflow-hidden" style={{ height: '300px', clipPath: 'url(#card-shape)' }}>
                            <img src={getImageUrl(stop.image)} alt={`${stop.name}, ${stop.city}`} className="h-full w-full object-cover" draggable={false} />
                          </div>
                        ...
                        <div className="relative w-full h-full">
                          <img src={getImageUrl(stop.image)} alt={`${stop.name}, ${stop.city}`} className="h-full w-full object-cover rounded-lg" draggable={false} />
