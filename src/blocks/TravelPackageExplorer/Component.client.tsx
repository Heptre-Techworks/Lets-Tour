...
const PackageCard: React.FC<{ pkg: any }> = ({ pkg }) => {
  const SuitabilityBar: React.FC<{
    ...
    <div className="mb-2">
      <div className="flex items-center">
        {typeof icon === 'string' ? <span className="text-xl mr-2">{icon}</span> : icon}
        <div className="ml-2">
          ...
          <div className="flex flex-col sm:flex-row">
            ...
            <div className="w-full sm:w-48 mt-4 sm:mt-0 flex-shrink-0">
              ...
              <div className="mt-4 space-x-0 sm:space-x-2 space-y-3">
                {pkg.categoryPills?.enablePills !== false &&
                  pkg.categoryPills?.pills?.map((pill: any, idx: number) => (
                    <SuitabilityBar
                      key={idx}
                      label={pill.title}
                      sublabel={pill.subtitle}
