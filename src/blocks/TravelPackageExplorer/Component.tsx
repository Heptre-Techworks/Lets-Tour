...
import type { TravelPackageExplorerBlock as TravelPackageExplorerBlockProps } from '@/payload-types';
...
const Filters: React.FC<{
  ...
  const accommodationTypes = [...new Set(packages.map((p) => p.accommodationType))];
  const amenities = [...new Set(packages.flatMap((p) => p.amenities))];
  const provinces = [...new Set(packages.map((p) => p.province))];
...
export const TravelPackageExplorer: React.FC<TravelPackageExplorerBlockProps> = ({
  packages = [],
}) => {
